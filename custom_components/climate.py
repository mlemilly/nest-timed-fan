...
class ThermostatEntity(ClimateEntity):
    ...
    def extra_state_attributes(self) -> dict[str, Any]:
        """Return the extra state attributes for the climate entity."""
        attributes = {}
        if FanTrait.NAME in self._device.traits:
            trait = self._device.traits[FanTrait.NAME]
            # Add timer end time if available
            if hasattr(trait, "timer_end_time") and trait.timer_end_time:
                attributes["fan_timer_end_time"] = trait.timer_end_time
            # Add current duration if available
            if hasattr(trait, "timer_duration") and trait.timer_duration:
                attributes["fan_timer_duration"] = trait.timer_duration
            # Add timer mode status
            if hasattr(trait, "timer_mode") and trait.timer_mode:
                attributes["fan_timer_mode"] = trait.timer_mode
        return attributes
    ...
    async def async_set_fan_mode(self, fan_mode: str) -> None:
        ...
        await self.async_set_fan_timer(fan_mode, duration=MAX_FAN_DURATION)

    async def async_set_fan_timer(
        self, fan_mode: str, duration: int | None = None
    ) -> None:
        """Set fan mode with custom duration in seconds."""
        ...
        timer_duration = None
        if fan_mode != FAN_OFF:
            timer_duration = duration if duration is not None else MAX_FAN_DURATION
        try:
            await trait.set_timer(FAN_INV_MODE_MAP[fan_mode], duration=timer_duration)
        except ApiException as err:
            raise HomeAssistantError(
